...
ServiceCard.Icon = ({ children }: { children: React.ReactNode }) => {
    return (
        <div className="relative z-20 flex items-center justify-center w-16 h-16 mx-auto bg-purple-400 rounded-lg dark:bg-purple-500 aspect-square text-c-light group-hover:text-white">
            {children}
        </div>
    )
}
...
ServiceCard.Body = ({ children }: { children: React.ReactNode }) => {
    return (
        <div className="flex flex-col w-full h-full px-4 py-8 -my-8 border rounded-lg bg-c-light border-c-gray-0 dark:border-c-dark0 dark:bg-c-dark2 dark:group-hover:bg-purple-950 group-hover:text-c-light group-hover:outline-purple-400 outline outline-transparent">
            {children}
        </div>
    )
}
